...
<h1>＜商品を検索画面＞</h1>
<div class="form">
    <form action="SelectItemServlet" method="post">
        <p>商品名:
        <select name="itemId" required></p>
        <option value="">選択してください</option>
        <c:forEach var="itemsdto" items="${itemlist}">
            <option value="${itemsdto.itemid}">${itemsdto.itemid}:${itemsdto.itemname}</option>
        </c:forEach>
        </select>
        <p>
        開始日付: <input type="date" name="startdate" required>
        <p>
        終了日付: <input type="date" name="enddate" required>
        <p>
